...
class PredDiff(object):
    ...
    def interactions(self, data_test: np.ndarray, list_interaction_masks: List[List[np.ndarray]] = None,
        ...
        for mask1, mask2 in list_interaction_masks:
            ...
            # get imputed predictions
            # TODO: deal with multiple weights for separate integrals
            ...
            mean_shielded_joined, low, high, _ = empirical_bootstrap(input_tuple=input_tuple, score_fn=eval_interaction,
                                                     n_iterations=n_bootstrap, fast_evaluation=self.fast_evaluation)
            mean_shielded_joined += predictions_true
            low += predictions_true
            high += predictions_true

            mean_joined = - mean_shielded_joined
            if individual_contributions is True:
                ...
                #mean01_overall = predictions_true - eval_mean(predictions01)
                #mean0_main = predictions_true - eval_mean(predictions0)
                #mean1_main = predictions_true - eval_mean(predictions1)
                ...
                res = [{'mean': m, 'high': h, 'low': l, 'mean01': m01, 'high01': h01, 'low01': l01, 'mean0': m0,'high0': h0, 'low0': l0, 'mean1': m1,'high1': h1, 'low1': l1, 'mean0shielded': m0s, 'high0shielded': h0s, 'low0shielded': l0s, 'mean1shielded': m1s,'high1shielded': h1s, 'low1shielded': l1s}
                       for m, h, l, m01, h01, l01, m0, h0, l0, m1, h1, l1, m0s, h0s, l0s, m1s, h1s, l1s in zip(mean_joined, high, low, mean01_overall, high01_overall, low01_overall, mean0_main, high0_main, low0_main, mean1_main, high1_main, low1_main, mean0_shielded, high0_shielded, low0_shielded, mean1_shielded, high1_shielded, low1_shielded)]
            else:
                res = [{'mean': m, 'high': h, 'low': l} for m, h, l in zip(mean_joined, high, low)]
    ...
    def interaction_matrix(self, data_test: np.ndarray = None, n_bootstrap=100) -> np.ndarray:
        """
        returns np.array containing all feature interaction for each sample.
        :param data_test: if given, all samples are interpreted, pd.DataFrame
        ...
        m_list_interaction = self.interactions(data_test=data_test)
        m_list_relevance = self.relevances(data_test=data_test)  # all features relevances
    ...
    def _setup_predict(self, data: np.ndarray):
        # TODO: simplify this function by promoting shape, n_samples to class member
    ...
    def _calc_predictions(self, data: np.ndarray, shape_imputations, mask_impute: np.ndarray,
        ...
        # shuffle second feature to explicitly enforce a factorizing imputer distribution
